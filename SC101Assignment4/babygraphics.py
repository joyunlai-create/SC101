...
def get_x_coordinate(width, year_index):
    ...
    """
    return ((width-2*GRAPH_MARGIN_SIZE)/len(YEARS))*year_index
...
def draw_fixed_lines(canvas):
    ...
    for i in range(len(YEARS)):
        ...
        canvas.create_line(get_x_coordinate(CANVAS_WIDTH, i+1), 0, get_x_coordinate(CANVAS_WIDTH, i+1), CANVAS_HEIGHT)
...
def draw_names(canvas, name_data, lookup_names):
    ...
    for name in lookup_names:
        for i in range(len(YEARS) - 1):
            ...
            canvas.create_text(x_1 + TEXT_DX, y_1, text=f'{name} {y_1_text}', anchor=tkinter.SW, fill=color)
            canvas.create_text(x_2 + TEXT_DX, y_2, text=f'{name} {y_2_text}', anchor=tkinter.SW, fill=color)

        # changes line color before starting drawing the next name
        color_index += 1
