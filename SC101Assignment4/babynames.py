...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name not in name_data:
        ...
        if year in name_data[name]:
            if name_data[name][year] > rank:    # if the current ranking is higher than in the dictionary
                name_data[name][year] = rank
        else:
            name_data[name][year] = rank
...
def search_names(name_data, target):
    ...
    """
    matching_names = []
    for babyname in name_data:
        if target.lower() in babyname.lower():
            matching_names.append(babyname)
